...
# _______________________________Beginn____________________________________
###########################################################################
...
with st.sidebar:
    ...
    # _______________________Berechnung der Tragfähigkeit_____________________
    ###########################################################################

    if 'Einlesen':

        # Berechnung aller Tragfähigkeiten mit Funktion
        # ec5_87_tragfähigkeit_vg()
        ...
        with tab_i:
            ...
            if nw[0] != 'success':
                ...
                for i in range(len(L_Li)):
                    ...
                    if (options_filter == ["Tragfähigkeit", "Längen"] or options_filter == [
                            "Längen", "Tragfähigkeit"]):
                ...
                for m, Frd_i in enumerate([L_di_axiali, L_di_scheri]):
                    ...
                    for p, Frd in enumerate(Frd_i):
                        ...
                        if options_filter == ["Tragfähigkeit"]:
                            ...
                            for k in range(len(L_dict[n])):
                                ...
                                # if-Abfrage zur Vermeidung der Division durch
                                # 0
                                if Frd_i[k] == 0:
                                    print("")
                                else:
                                    annotations.append(
                                        go.Annotation(
                                            text=str(f"{round(L_F[m]/Frd_i[k],2):.0%}"),
                                            x=k + 1.5,
                                            y=Frd_i[k] + y_max / 20,
                                            xref='x',
                                            yref='y',
                            ...
                            # Dynamische Anpassung der Farben entsprechend der
                            # Filter
                            if Frd > L_F[m]:
                    ...
                    fig.update_traces(
                        marker_color=L_colors_bar[m],
                        textfont=L_font_prozent[m])
                    fig.update_layout(annotations=annotations)
                    fig.update_yaxes(title_text="Tragfähigkeit [kN]")
                    fig.update_xaxes(
                        showticklabels=False,
                        tickangle=90,
                        title_text="Längen [mm]",
                        type='linear')
...
if betrachtung == 'Einzelnachweis':
    ...
    if button_axial:
        ...
        # LATEX
        st.latex(
            r''' F_{axRk}  =  k_{axk} * f_{axk} * l_{ef} * \left(\frac{\rho_{k}}{350} \right)^{0.8} = ''' +
            str(F_axRk1) +
            ' kN')
        ...
        st.latex(
            r'''F_{headRk}  = f_{head} * d_{h}^2 * \left(\frac{\rho_k}{350} \right)^{0.8} = ''' +
            str(F_headRk) +
            ' kN')
        ...
        if t_Blech == 0:
            ...
            F_vk5 = 1.05 * (f_hk * t_2 * d) / (3) * (sqrt(4 +
                                                          (12 * M_yrk) / (f_hk * t_2**2 * d)) - 1) + F_axrk / 4
            ...
            st.latex(
                r''' F_{vk1} = f_{hk}*t_{1}*d =  ''' +
                str(F_vRk1) +
                ' kN')
            st.latex(
                r''' F_{vk2} = f_{hk}*t_{2}*d =  ''' +
                str(F_vRk2) +
                ' kN')
            st.latex(
                r''' F_{vk3} = \frac{f_{hk}*t_{1}*d}{1+\beta}*\left(\sqrt{1+2\beta^2*(1+\frac{t_2}{t_1}+\left(\frac{t_2}{t_1}\right)^2)+\beta^3\left(\frac{t_2}{t_1}\right)^2} - \beta\left(1+\frac{t_2}{t_1}\right)\right) + \frac{F_{axRk}}{4} = ''' +
                str(F_vRk3) +
                ' kN')
            st.latex(
                r''' F_{vk4} = 1.05 * \frac{f_{hk}*t_{1}*d}{2+\beta}*\left(\sqrt{2\beta*(1+\beta)+\frac{4*\beta*(2+\beta)*M_{yRk}}{f_{hk}*d*t_{1}^2}}-\beta\right) + \frac{F_{axRk}}{4} = ''' +
                str(F_vRk4) +
                ' kN')
            st.latex(
                r''' F_{vk5} = 1.05 * \frac{f_{hk}*t_{2}*d}{1+2\beta}*\left(\sqrt{2\beta^2*(1+\beta)+\frac{4*\beta*(1+2\beta)*M_{yRk}}{f_{hk}*d*t_{2}^2}}-\beta\right) + \frac{F_{axRk}}{4} = ''' +
                str(F_vRk5) +
                ' kN')
            st.latex(
                r''' F_{vk6} = 1.15 * \sqrt{\frac{2\beta}{1+\beta}}*\sqrt{2*M_{yRk}*f_{hk}*d} + \frac{F_{axRk}}{4} = ''' +
                str(F_vRk6) +
                ' kN')
        ...
        # dickes Stahlblech
            ...
            st.latex(
                ...
                ' kN')
            st.latex(
                r''' F_{vk2} = f_{hk}*t_{2}*d * \left( \sqrt{2+ r( \frac{4*M_{yRk}}{f_{hk}*d*t_{1}^2}} ) \right) + \frac{F_{axRk}}{4} = ''' +
                str(F_vRk2) +
                ' kN')
            st.latex(
                r''' F_{vk3} = 2.3 * \sqrt{2*M_{yRk}*f_{hk}*d} + \frac{F_{axRk}}{4} = ''' +
                str(F_vRk3) +
                ' kN')
            st.latex(
                r'''  F_{vRk} = min( ''' +
        ...
        # dünnes Stahlblech
            ...
            st.latex(
                r''' F_{vk1} = 0.4 * f_{hk}*t_{1}*d =  ''' +
                str(F_vRk1) +
                ' kN')
            st.latex(
                r''' F_{vk2} = 1.15 * \sqrt{2*M_{yRk}*f_{hk}*d} + \frac{F_{axRk}}{4} = ''' +
                str(F_vRk2) +
                ' kN')
            st.latex(
                r'''  F_{vRk} = min( ''' +
